...
function toggleFullscreenMap(value){
	...
	if(value){
		...
		$('html').addClass('fullScreen');
                $('.topbar, .headerContainer, .pagination').hide(); 
	...
	// EXIT FULLSCREEN
		...
		$('html').removeClass('fullScreen');
                $('.topbar, .headerContainer, .pagination').show(); 
		// SET BUTTTON CLASSES AND TEXT
		$('#fullScreen').attr('title',buttonText);
		// TOGGLE GLOBAL VARIABLE
		mapFullscreen = 0;
...
$(document).ready(function() {
	...
	$(document).on('click','#showLayers',function(event) {
                console.log("here"); 
		tabMenu('#layerMenu',this);
	});
	/*------------------------------------*/
	...
	$('#searchAddress').keyup(function(e){
		...
		if(e.keyCode == 13 && $(this).val()!==$searchDefault && $(this).val()!=='') {
			// clearTimeout (timer); @TODO Fix this ESRI, crashes Chrome.
			clearLocate();
			locate();
			$('#autoComplete').hide();
		}
	...
	$(document).on('click','#autoComplete ul li',function(event) {
		...
		// showResults(ACObj, locNum);
                showResultsProjected(ACObj, locNum);
		$('#autoComplete').hide();
	});
...
function insertEmbedHidden(){
	...
	html += '</div>';
	html += '</div>';
	html += '<table class="embedOptions" id="embedTable">';
	html += '<tr><td>';
	html += '<input id="showLegendEmbed" type="checkbox" checked>';
	html += '</td><td><label for="showLegendEmbed">Show Legend</label></td>';
	html += '</tr><tr><td>';
	html += '<input id="showAboutEmbed" type="checkbox" checked></td><td>';
	html += '<label for="showAboutEmbed">Show About this Map</label></td>';
	html += '</tr><tr>';
	html += '<td>';
