	...
	if(totalItems > 0){
		...
		for(var i=0; i < forTotal; ++i) {
			...
			if(obj.layout == 'list'){
				...
				html += '<a id="mapItem'+i+'" title="' + snippet + '" href="' + itemURL + '"><img src="' + obj.arcgispath + obj.imagepath + data.results[i].id + obj.imagepath2 + data.results[i].thumbnail + '" width="100" height="66"><span>' + itemTitle + '</span></a>';
				html += '</li>';
...
function configLayoutSearch(){
	if(pmgConfig.showGroupSearch || pmgConfig.showLayoutSwitch){
		...
		if(pmgConfig.showGroupSearch){
			html += '<li id="mapSearch" class="iconInput"><div title="Clear" class="iconReset" id="clearAddress"></div><input placeholder="Search map gallery" id="searchGroup" title="Search map gallery" value="" autocomplete="off" type="text" tabindex="1"></li><li title="Search" class="searchButtonLi"><span id="searchGroupButton" class="silverButton buttonRight" style="height:17px;"><span class="searchButton"></span></span> </li>';	
		}
		if(pmgConfig.showLayoutSwitch){
			switch(pmgConfig.defaultLayout){
                                case 'slide': 
                                        var slideClass = 'active';
					var listClass = '';
					var gridClass = '';	
					break;
			...
			var switchHTML = '<ul class="switchList"><li class="toggleLayout"><ul ><li id="layoutSlide" class="'+slideClass+'" title="Switch to Slide View"><span class="silverButton buttonLeft"><span class="slideView"></span></span></li><li id="layoutGrid" class="'+gridClass+'" title="Switch to Grid View"><span class="silverButton buttonLeft"><span class="gridView"></span></span></li><li id="layoutList" class="'+listClass+'" title="Switch to List View"><span class="silverButton buttonRight"><span class="listView"></span></span></li></ul></li></ul>';	
                        $("#layoutSwitch").html(switchHTML); 
		}
		html += '</ul><div class="clear"></div>';
...
$(document).ready(function() {
	...
	$(document).on('click','#clearAddress, #resetGroupSearch',function(event) {
		...
		prevVal = searchVal;
		$('#groupAutoComplete').hide();
                $(".pagination, .headerContainer").hide(); 
	});
	// LIST VIEW
	...
	// SLIDE VIEW
	$(document).on('click','#layoutSlide',function(event) {
		if(layoutType != 'slide'){
			layoutType = 'slide';
			$('.toggleLayout li').removeClass('active');
			$(this).addClass('active');
	...
	$('#searchGroup').keyup(function(e){
                $(".pagination, .headerContainer").show(); 
		var aquery = $(this).val();
